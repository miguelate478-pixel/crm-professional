...
export default function AcceptInvitePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) { toast.error("Token de invitación inválido"); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Error al aceptar la invitación");
      }
    } catch {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
        <div className="text-center text-white">
          ...
          <p className="text-slate-400 mb-4">El enlace de invitación no es válido o ha expirado.</p>
          <a href="/login" className="text-blue-400 hover:text-blue-300">Ir al inicio de sesión</a>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-slate-900 border border-slate-800/60 rounded-2xl p-8 shadow-2xl">
          <h2 className="text-lg font-semibold text-white mb-1">Aceptar invitación 🎉</h2>
          <p className="text-slate-400 text-sm mb-6">Completa tu perfil para unirte al equipo.</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-slate-300 text-sm">Tu nombre completo</Label>
              <Input
                type="text"
